...
export default function Login (props : any) {
      ...
      axios
          ...
          .then((res: any)=>{
            if (res.data.isUser === true) {
              AsyncSetItem(res.data.refreshToken, res.data.userAccount, res.data.userName, res.data.userURL);
  ...
  // 네이버 로그인
  const consumerKey = 'rTNXa8pFQ1b6E5jV2B6t';
  const consumerSecret = 'f7tecQTacF';
  const appName = 'com.erounpamphlet.app';
  const serviceUrlScheme = 'com.erounpamphlet.app';
    ...
    <View style={Platform.OS === 'android' ? styles.android : styles.ios}>
      <View style={styles.container}>
        <View style={styles.mainlogo}>
          <Image source={require('../images/login/loginlogo.png')} style={{width:250, height:250, resizeMode:"contain"}}/>
        </View>

        <TouchableOpacity
          style={{width:250, height:50, backgroundColor:"#EAEAEA", justifyContent:'center', alignItems:'center', borderRadius:15}}
          onPress={()=>{
            props.navigation.replace('Navi_Main');
          }}
        >
          <Typography>demo version login</Typography>
        </TouchableOpacity>
